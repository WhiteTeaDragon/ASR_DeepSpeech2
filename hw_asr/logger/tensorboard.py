...
class TensorboardWriter(Writer):
    def __init__(self, log_dir, logger, enabled):
        ...
        if enabled:
            ...
            if not succeeded:
                message = (
                    "Warning: visualization (Tensorboard) is configured to "
                    "use, but currently not installed on this machine. "
                    "Please install TensorboardX with 'pip install "
                    "tensorboardx', upgrade PyTorch to version >= 1.1 to use "
    ...
    def __getattr__(self, name):
        ...
        tensorboard with additional information (step, tag) added.
        Otherwise: return a blank function handle that does nothing
        """
